...
// governance-only for the same reason every chain-wide grant is. The authority
// behind the act did not move — a chain-wide grant is governance's alone. What
// moved is where it is recorded, and that is the point: the appointment and the
// role registry are one mechanism now rather than two lists that happened to
// share a name.
//
// Two things were wrong with the list beyond the whole-object hazard above, and
// they are what the move was for. Validate() never checked that an entry was an
// address, so a mistyped one passed a vote, occupied one of the eight places and
// granted the exemption to nobody; MsgGrantRole decodes the holder. And an
// administrator was a bare address, where a role holder must be an x/group
// account — so the exemption is now held M-of-N like every other authority on
// this chain.
...
type MsgClient interface {
	...
	// UpdateParams sets the module parameters. Governance only, and since the
	// administrator list moved to the role registry it sets exactly one.
