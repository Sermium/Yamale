...
// AppModule implements the module interface for x/tokenisation.
//
// No BeginBlocker and no EndBlocker. Sales finalise through a permissionless
// crank rather than a per-block sweep: iterating every reported sale each block
// is a denial-of-service surface that grows with usage, and the outcome is
// fully determined by the report and the clock, so it does not matter who pays
// for it or when.
