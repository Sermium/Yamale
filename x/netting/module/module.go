...
// EndBlock closes the netting window when one falls due and settles it.
//
// In EndBlock rather than BeginBlock so that an obligation submitted in the
// window's last block is in that window. Closing at the start of a block would
// settle a window that was still accepting traffic moments earlier, and no
// participant's back office could reconcile against a boundary like that.
