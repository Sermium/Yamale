...
**Running this for real.** `tools/feeder` is the daemon: it asks the chain which
denoms it accepts, reads prices from a source you name, submits once per vote
period, and exits non-zero on a round it could not complete.

```bash
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -o yamale-feeder ./tools/feeder
...
yamale-feeder \
  ...
  --from <feeder-key> \
  --validator <ymlvaloper1...> \
  --source erapi \
  --interval 60s
```

`deploy/systemd/yamale-feeder.service` runs it under systemd. Use it, and
`enable` it — a feeder that does not come back from a reboot is silent in
exactly the way a stopped feeder is silent, and the oracle's answer to a price
nobody is reporting is to mark it stale rather than to complain.

**One per validator, and not all on one source.** The oracle takes a
stake-weighted median, so a median of four reports drawn from one endpoint is
one source counted four times. `--source erapi` and `--source yahoo` are two;
the interface is one method, so a third is a small file.

**What it reports is what it observed, and nothing else.** A round where the
source returned nothing usable submits nothing at all, rather than resubmitting
the last good number. That deliberately costs a miss on the validator's counter:
a feeder that fills a gap with yesterday's price is indistinguishable from a
working one right up until the median it fed was wrong, and the miss counter is
the only place that difference ever becomes visible.

It also waits for its transaction to be in a block before reporting success.
That distinction is not pedantic here — an unauthorised feeder, a denom outside
the accepted set, and an account with a stale sequence all broadcast cleanly and
fail in the block. A feeder that logs success through all three is a feeder
whose output means nothing, which is a poor property for the one component whose
entire job is to be believed about what it saw.

What the chain guarantees, either way, is that the rules a client applies to
decide whether to show a price are the same rules the chain applies to decide
whether to lend against it.
