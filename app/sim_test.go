...
func TestFullAppSimulation(t *testing.T) {
	...
	require.Equal(t, "blockchain", app.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	skipPrefixes := map[string][][]byte{
		...
		// The oracle's in-flight votes are deliberately not exported: a voting
		// round that was open when the chain stopped describes prices from a
		// moment that has passed, and resuming it would agree a rate on reports
		// nobody currently stands behind. The round is abandoned and the next
		// one starts empty, so the imported chain has fewer keys here by design.
		// Everything the oracle actually stands behind — agreed rates,
		// valuations, delegations, miss counters — is compared as normal.
		oraclemoduletypes.StoreKey: {oraclemoduletypes.VoteKey.Bytes()},
	}
