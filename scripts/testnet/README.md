...
> Better: generate the operator key on a machine with no network, the same way
> the custodians do, and carry it across as an encrypted export.
>
> ```bash
> ceremony validator --name "<your institution>" --armor operator.asc
> blockchaind keys import validator operator.asc --keyring-backend file
> ```
>
> This is the **operator** key. The **consensus** key was generated by
> `blockchaind init` above, lives in `config/priv_validator_key.json`, and must
> never be copied anywhere — two copies signing at once is double-signing, which
> the chain slashes and jails with no appeal and no way to tell a restored
> backup from an attack. See
> [the key ceremony](../../docs/guides/key-ceremony.md).

## 1b. Hold the key ceremony (before step 2, not after)

The foundation account — `x/enforcement`'s `recovery_destination`, where every
seized asset is sent — is a **3-of-5 `x/group` policy**, not a key. It is built
in a room, on paper, by five custodians, and it has to exist before the genesis
is assembled.

Full runbook: **[the key ceremony](../../docs/guides/key-ceremony.md)**. It
covers the roles, the machine preparation, the transcription check, the restore
drill, and what to do when a phrase is exposed mid-ceremony.

What step 2 needs from it is one directory containing `group-genesis.json` and
`constitution-invariants.json`.

> **The ordering is not a preference.** `x/constitution`'s `InitGenesis` refuses
> to start a chain whose recovery destination is unset, so a genesis built
> before the ceremony fails on every validator at height 1. And the group has to
> be in the genesis file rather than created by a transaction afterwards: the
> address `x/group` derives for a policy account depends on the policy sequence
> number alone — not on the members, the threshold, the admin or the chain id —
> so naming the address while creating the group later would point every future
> seizure at whichever group policy somebody created first.
>
> The address is knowable offline, before genesis (`ceremony address --seq 1`
> prints it). What is *not* knowable from the address is who controls it, which
> is why both go into the same file.
...
`CEREMONY_DIR` is required and the script refuses to run without it. From it,
step 2:

- splices the foundation's group, its five members and its 3-of-5 policy into
  `app_state.group`;
- writes the policy address into **both** `app_state.enforcement.params
  .recovery_destination` and `app_state.constitution.invariants
  .enforcement_recovery_destination`, which the chain refuses to start with in
  disagreement;
- sets `foundation_custodian_count` and `foundation_signature_threshold` from
  the same ceremony output, so the constitution and the group cannot describe
  different arrangements;
- gives the policy address a genesis account, because `x/group`'s `InitGenesis`
  imports its own tables and does not create one.

The recovery destination used to be an optional environment variable, on the
argument that a chain which can freeze but not seize is a safe launch. That
argument did not survive contact with the devnet, which ran for weeks with the
parameter empty: what an empty destination actually buys is a chain where two
thirds of the validator set can pass a seizure that then has nowhere to send
what it took, and nobody notices until something prints the parameter. See
[the enforcement guide](../../docs/guides/enforcement.md).

The verification block at the end of the script checks the foundation as well as
the emission schedule, and for the same reason: every one of these values is
individually legal and collectively wrong in ways `genesis validate` cannot see.
A constitution saying three-of-five over a seeded group that is a two-of-four
starts a chain perfectly happily, and the ante gate would then spend its life
refusing every legitimate change to a group that was already the wrong shape.

The issuer named in step 2b can be the foundation policy address, or a separate
operational account — they no longer have to be the same, because the foundation
is now a group rather than a key somebody has to hold.
